...
object InMemoryWavWriter {
    ...
    fun write(file: File, samples: ShortArray, sampleRate: Int = SystemAudioCapture.SAMPLE_RATE) {
        ...
        writeInt32Le(buf, 16, 16)                              // sub-chunk size
        writeInt16Le(buf, 20, 1)                               // PCM
        writeInt16Le(buf, 22, CHANNELS)
        writeInt32Le(buf, 24, sampleRate)
        writeInt32Le(buf, 28, sampleRate * CHANNELS * BYTES_PER_SAMPLE) // byte rate
        writeInt16Le(buf, 32, CHANNELS * BYTES_PER_SAMPLE)     // block align
